...
#include "PumpkinBox/Utils/stb_image.h"
...
namespace pb {
		...
		:m_Width(width), m_Height(height)
	{
		...
		#elif defined(PB_PLATFORM_MACOS)
		...
		:m_Path(path), m_Width(0), m_Height(0)
	{
		...
		// PB_ASSERT(data, "Failed to load image!");
		if(!data) {
			PB_INFO("Failed to load image!");
			// PB_INFO("Current working directory: '{0}'", GetCurrentWorkingDir());
			return;
		}
		...
		PB_ASSERT(m_InternalFormat && m_DataFormat, "Image format trying to load is not supported!");
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		PB_ASSERT(size == m_Width * m_Height * bytes_per_channel, "Data passed in OpenGLTexture2D::SetData must be entire texture! Missing values!");
	...
	/*void OpenGLTexture2D::SetMagnification(TextureMag textMag)
	{
		...
		default:
			PB_ASSERT(false, "Unknown texture magnification setting!");
		}
	}
	...
	void OpenGLTexture2D::SetMinification(TextureMag textMag)
	{
		...
		default:
			PB_ASSERT(false, "Unknown texture minification setting!");
		}
	}
*/
